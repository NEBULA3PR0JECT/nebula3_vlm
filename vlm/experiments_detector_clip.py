...
class CLIP_OBJECT_DETECTOR:
    ...
    def clip_persons_expert(self, frame, topk):
        ...
        sorted_ppl_texts, ppl_scores = self.get_nn_text(ppl_texts, ppl_feats, img_feats, 0)
        ppl_result = sorted_ppl_texts[0]
        sorted_ppl_texts, ppl_scores = self.get_nn_text(self.people_texts, self.people_feats, img_feats, 0.15)
        ...
        if (fn):
            ...
            if video_file.is_file():
                ...
                for scene_element, data in enumerate(metadata['scene_elements']):
                    ...
                    persons_ = []
                    number_of_ppl = []
                    for mdf in tqdm(range(mdfs[0], mdfs[2])):
                    #for mdf in tqdm(range(first_frame, last_frame)):
                        ...
                        scale_down_x = 0.4
                        scale_down_y = 0.4
                        frame_rgb = cv2.cvtColor(_frame_, cv2.COLOR_BGR2GRAY)
                        frame_res = cv2.resize(frame_rgb, None, fx= scale_down_x, fy= scale_down_y, interpolation= cv2.INTER_AREA)
                        if not ret:
                            print("File not found")
                        else:
                            #mdf_experts = self.clip_expert(frame_rgb, 3, 10)
                            good_frame, pcl, ppl, obj = self.mdf_selection(frame_res)
                    ...
                    sorted_persons = list(islice(sorted_persons, 5))
                ...
                return(list(islice(sorted_locations, 3)), scene_number_of_ppl, scene_persons)
...
def main():
    ...
    res = cod.clip_experts_for_scene_element('Movies/222510046')
